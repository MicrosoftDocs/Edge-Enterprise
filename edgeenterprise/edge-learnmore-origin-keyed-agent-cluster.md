...
After Microsoft Edge inherits the change from Chromium, attempts to modify the `document.domain` property using JavaScript will be ignored. We anticipate this change to be in Edge stable 118 or later. You'll need to use alternative approaches, such as `postMessage()` or the Channel Messaging API, to communicate cross-origin.
