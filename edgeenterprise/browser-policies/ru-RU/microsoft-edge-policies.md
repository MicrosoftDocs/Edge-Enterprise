...
```
  ...
  #### Описание
  Сначала проверяются [PluginsAllowedForUrls](#pluginsallowedforurls) и [PluginsBlockedForUrls](#pluginsblockedforurls), а затем эта политика. Возможные значения: "ClickToPlay" и "BlockPlugins". Если присвоить этой политике значение "BlockPlugins", этот подключаемый модуль запрещается для всех веб-сайтов. "ClickToPlay" позволяет запустить подключаемый модуль Flash, но для его запуска пользователи должны нажать заполнитель.

Если не настроить эту политику, пользователь сможет изменить этот параметр вручную.

Примечание. Автоматическое воспроизведение возможно только для доменов, явно перечисленных в политике [PluginsAllowedForUrls](#pluginsallowedforurls). Чтобы включить автоматическое воспроизведение для всех сайтов, добавьте http://* и https://* в список разрешенных URL-адресов.
...
```
  ...
  ## Параметры режима терминала policies

  [В начало](#microsoft-edge:-политики)

  ### KioskDeleteDownloadsOnExit
  #### Удалять файлы, скачанные в рамках сеанса терминала, когда Microsoft Edge закрывается
  
  
  #### Поддерживаемые версии:
  - На Windows с 87 или более поздней версии

  #### Описание
  Примечание. Эта политика поддерживается только при запуске Microsoft Edge с параметром командной строки "--edge-kiosk-type".

Если эта политика включена, при каждом закрытии Microsoft Edge файлы, скачанные в рамках сеанса терминала, удаляются.

Если эта политика отключена или не настроена, при закрытии Microsoft Edge файлы, скачанные в рамках сеанса терминала, не удаляются.

Подробные сведения о настройке режима терминала см. на странице [https://go.microsoft.com/fwlink/?linkid=2137578](https://go.microsoft.com/fwlink/?linkid=2137578).
  ...
  #### Настройки и сведения Windows
  ##### Сведения о групповой политике (ADMX)
  - Уникальное имя групповой политики: KioskDeleteDownloadsOnExit
  - Имя групповой политики: Удалять файлы, скачанные в рамках сеанса терминала, когда Microsoft Edge закрывается
  - Путь групповой политики (Обязательно): Административные шаблоны/Microsoft Edge/Параметры режима терминала
  - Путь групповой политики (Рекомендовано): Н/Д
  - Имя файла групповой политики ADMX: MSEdge.admx
  ##### Настройки реестра Windows
  - Путь (Обязательно): SOFTWARE\Policies\Microsoft\Edge
...
Typically, this is disabled as a phishing defense. If you don't configure this policy, it's disabled and third-party images can't show an authentication prompt.
  ...
  - Имя групповой политики: Allow cross-origin HTTP Authentication prompts
...
политика [BrowserSignin](#browsersignin) не должна быть настроена или должна быть включена. Если политика [BrowserSignin](#browsersignin) отключена, политика [ForceSync](#forcesync) не будет применяться.

Политика [SyncDisabled](#syncdisabled) не должна быть настроена или ей должно быть присвоено значение False. Если ей присвоено значение True, политика [ForceSync](#forcesync) не будет применяться.
...
```
  ...
  #### Описание
  Если эта политика включена, интерфейс первого запуска и экран-заставка не будут отображаться пользователям при запуске Microsoft Edge в первый раз.

Для настроек конфигурации, отображаемых в интерфейсе первого запуска, браузер присвоит следующие стандартные значения:

– На странице новой вкладки для типа веб-канала будет настроено значение "MSN Новости", для макета — "Для вдохновения".

– Для пользователя по-прежнему будет выполняться автоматический вход в Microsoft Edge, если учетная запись Windows относится к типу Azure AD или MSA.

– Синхронизация не будет включена по умолчанию, и пользователям будет предлагаться указать, следует ли выполнять синхронизацию при запуске браузера. Вы можете использовать политику [ForceSync](#forcesync) или [SyncDisabled](#syncdisabled), чтобы настроить синхронизацию и запрос на продолжение синхронизации.
...
Если эта политика не настроена, пользователь сможет изменить этот параметр.
...
Если данный параметр выключен или не настроен, пользователи могут отключать изоляцию сайтов. (Например, с помощью записи "Отключить изоляцию сайта" на странице edge://flags.) Если выключить или не настроить данный параметр, это не приведет к выключению функции "Изоляция сайта".
...
```
  ...
  #### Описание
  Настройте эту политику, чтобы указать список веб-приложений, которые устанавливаются автоматически без взаимодействия с пользователем и не могут быть отключены или удалены пользователями.

Каждый элемент списка политики является объектом с обязательным элементом: url (URL-адрес веб-приложения для установки) и 2 необязательными элементами: default_launch_container (указывает режим окна, с помощью которого открывается веб-приложение, — параметром по умолчанию является новая вкладка) и create_desktop_shortcut (значение true, если вы хотите создать Linux и ярлыки рабочего стола Windows).
