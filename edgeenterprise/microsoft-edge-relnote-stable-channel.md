...
- **Microsoft Edge: Inline Compose.** This feature lets users rewrite text using Copilot. A user can highlight text they'd like to improve when writing in Edge and use the Rewrite menu to try different lengths, formats, and tones. Administrators can control availability using the [ComposeInlineEnabled](/deployedge/microsoft-edge-policies#composeinlineenabled) policy.
