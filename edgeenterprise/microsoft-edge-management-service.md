...
Microsoft Edge policy has the concept of the audience that the policy is meant to apply to, this can be either "User" or "Device". In Microsoft Edge Management Service, the policy applied via Group Assignment is applied as User Policy, while policy pulled down via [EdgeManagementAdminCenterEnrollmentToken] is applied as Device Policy.
